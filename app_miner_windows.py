...
MINER_PATH = "C:\\xmrig\\xmrig.exe"  # Caminho absoluto para o executável do minerador (neste caso, xmrig para Windows).

# Verificação do sistema operacional
def check_os():
    """
    Função para verificar se o sistema operacional é Windows.
    Se não for, exibe uma mensagem de erro e encerra o programa.
    """
    system = platform.system()  # Obtém o nome do sistema operacional atual (ex: 'Windows', 'Linux', 'Darwin' para macOS).
    if system != "Windows":  # Verifica se o sistema operacional é diferente de Windows.
        print("Este script é projetado para Windows. O sistema atual é:", system)  # Exibe uma mensagem indicando o sistema operacional incompatível.
...
def start_miner():
    ...
    try:
        ...
        subprocess.run(miner_command, shell=True)  # Executa o comando do minerador no sistema operacional.
